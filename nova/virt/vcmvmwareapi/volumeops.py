...
"""
import re # Vsettan-only
import string # Vsettan-only
from operator import itemgetter # Vsettan-only
...
class VMwareVolumeOps(object):
    ...
    def __init__(self, session, cluster=None,
        ...
        self._resource_pool = resource_pool # Vsettan-only
    ...
    # Vsettan-only start
    def _get_host_by_instance(self, instance=None):
        """If the instance is not None, return the host mor in which the
           instance resides. If the instance is None, return the first host of
           the cluster.
        """
        if instance:
            host_mor = vm_util.get_host_ref_for_vm(self._session, instance)
        else:
            # use the first host of the cluster
            host_mor = vm_util.get_host_ref(self._session, self._cluster)
        return host_mor

    def _iscsi_build_static_target(self, data):
        client_factory = self._session.vim.client.factory
        target_portal = data['target_portal']
        # create HostInternetScsiHbaAuthenticationProperties object
        auth_prop = client_factory.create(
            'ns0:HostInternetScsiHbaAuthenticationProperties')
        if 'auth_method' in data and data['auth_method'] == 'CHAP':
            auth_prop.chapAuthEnabled = True
            auth_prop.chapAuthenticationType = 'chapRequired'
            auth_prop.chapInherited = False
            auth_prop.chapName = data['auth_username']
            auth_prop.chapSecret = data['auth_password']

        # create HostInternetScsiHbaStaticTarget object
        static_target = client_factory.create(
            'ns0:HostInternetScsiHbaStaticTarget')
        static_target.authenticationProperties = auth_prop
        (static_target.address, static_target.port) = target_portal.split(':')
        static_target.discoveryMethod = 'sendTargetMethod'
        static_target.iScsiName = data['target_iqn']
        static_target.parent = target_portal
        return static_target

    def _iscsi_add_static_target(self, data, instance=None):
        host_mor = self._get_host_by_instance(instance)
        ...
        for hba in host_hbas:
            ...
            return
        static_target = self._iscsi_build_static_target(data)
        self._session._call_method(self._session.vim,
            "AddInternetScsiStaticTargets", storage_system_mor,
            iScsiHbaDevice=hba_device, targets=static_target)

    def _iscsi_remove_static_target(self, data, instance=None):
        host_mor = self._get_host_by_instance(instance)
        storage_system_mor = self._session._call_method(
            vim_util, "get_dynamic_property",
            host_mor, "HostSystem",
        ...
        static_target = self._iscsi_build_static_target(data)
        self._session._call_method(self._session.vim,
            "RemoveInternetScsiStaticTargets", storage_system_mor,
            iScsiHbaDevice=hba_device, targets=static_target)
    # Vsettan-only end

    def _iscsi_get_target(self, data, instance=None):  # Vsettan-only
        """Return the iSCSI Target given a volume info. If instance is None,
           will return the iscsi target on the first host of the cluster.
        """
        target_portal = data['target_portal']
        target_iqn = data['target_iqn']
        host_mor = self._get_host_by_instance(instance) # Vsettan-only
    ...
    def _iscsi_rescan_hba(self, target_portal, instance=None):  # Vsettan-only
        """Rescan the iSCSI HBA to discover iSCSI targets. If the instance is None,
           will scan the hba on the first host of the cluster
        """
        host_mor = self._get_host_by_instance(instance)
        storage_system_mor = self._session._call_method(
                vim_util, "get_dynamic_property",
        ...
        for hba in host_hbas:
            if hba.__class__.__name__ == 'HostInternetScsiHba':
                hba_device = hba.device
                # Vsettan-only. Remove the dymanic target host here, because we
                # will manually add static target with CHAP authentication.
###                if target_portal:
...
###                                                         target_portal)
    ...
    def _iscsi_discover_target(self, data, instance=None): # Vsettan-only
        ...
        device_name, uuid = self._iscsi_get_target(data, instance)  # Vsettan-only
        if device_name:
            LOG.debug("Storage target found. No need to discover")
            return (device_name, uuid)

        # Add iSCSI static target
        self._iscsi_add_static_target(data, instance)   # Vsettan-only

        # Rescan iSCSI HBA with iscsi target host
        self._iscsi_rescan_hba(target_portal, instance)   # Vsettan-only
    ...
    def _iscsi_get_host_iqn(self, instance=None):   # Vsettan-only
        """Return the host iSCSI IQN. If instance is None, the iqn of the
           first host of the cluster will be returned.
        """
        host_mor = self._get_host_by_instance(instance)   # Vsettan-only
        hbas_ret = self._session._call_method(
            vim_util, "get_dynamic_property",
    ...
    def get_volume_connector(self, instance):
        ...
        try:
            ...
            vm_ref = None
        iqn = self._iscsi_get_host_iqn(instance if vm_ref else None)  # Vsettan-only
    ...
    def _attach_volume_iscsi(self, connection_info, instance,
        ...
        device_name = self._iscsi_discover_target(data, instance)[0]   # Vsettan-only
    ...
    def _detach_volume_iscsi(self, connection_info, instance):
        ...
        if device is None:
            raise exception.DiskNotFound(message=_("Unable to find volume"))
        # Remove iSCSI static target
        self._iscsi_remove_static_target(data, instance)    # Vsettan-only
    ...
    def get_disks_info(self, instance, volume_uuids):
        ...
        # Get disk details from instance ref
        disk_devices = self._get_virtual_disk_devices(vm_ref)

        disks = []

        for device in disk_devices:
            # Check if this disk is in the VM folder
            diskFileName = device.backing.fileName
            diskFolder = diskFileName[:diskFileName.rfind("/")]
            in_vm_folder = diskFolder == instance['vmFolder']

            # Check if this disk is a volume
            is_vol = False
            if not in_vm_folder:
                for vol_uuid in volume_uuids:
                    if vol_uuid in device.backing.fileName:
                        is_vol = True
                        break

            # Parse out the disk number from the disk label, e.g. "Hard disk 2"
            disk_num = 0
            match = re.search("Hard disk ([\\d]*)", device.deviceInfo.label)
            if match:
                disk_num = int(match.group(1))

            capacity_gb = device.capacityInKB / 1024 / 1024

            disks += [{'in_vm_folder': in_vm_folder,
                       'is_vol': is_vol,
                       'disk_num': disk_num,
                       'capacity_gb': capacity_gb}]

        # Sort disks by disk number
        disks.sort(key=itemgetter('disk_num'))

        # Update instance with root and ephemeral disk information
        vm_disks = 0
        for disk in disks:
            if disk['in_vm_folder']:
                vm_disks += 1
                if vm_disks == 1:
                    instance['root_gb'] = disk['capacity_gb']
                    instance['root_device_name'] = self._get_disk_device_name(disk['disk_num'])
                elif vm_disks == 2:
                    instance['ephemeral_gb'] = disk['capacity_gb']
                    instance['ephemeral_device_name'] = self._get_disk_device_name(disk['disk_num'])

        return disks

    def _get_virtual_disk_devices(self, vm_ref):
        hardware_devices = self._session._call_method(vim_util,
                        "get_dynamic_property", vm_ref,
                        "VirtualMachine", "config.hardware.device")
        return vm_util.get_virtual_disks(hardware_devices)

    def _get_disk_device_name(self, disk_num):
        index = disk_num - 1
        if index >= 0 and index < len(string.ascii_lowercase):
            return "/dev/sd" + string.ascii_lowercase[index]
        else:
            return ""
    # Vsettan-only end

    #Vsettan hot resize begin
    def getRootDisk(self, instance):
        """get root disk device for an instance"""
        vm_ref = vm_util.get_vm_ref(self._session, instance)
        ...
        vm_path = self._session._call_method(vim_util,
                    "get_dynamic_property", vm_ref, "VirtualMachine", "config.files.vmPathName")
        vm_folder = vm_path[:vm_path.rfind("/")]
        disks = []
        #disk device key : iscsi controller
        disk_scsi_controller_mapping = {}
        def build_disks(device, disks):
            # Check if this disk is in the VM folder
            diskFileName = device.backing.fileName
            diskFolder = diskFileName[:diskFileName.rfind("/")]
            in_vm_folder = diskFolder == vm_folder
            # Check if this disk is assigned from local datastore
            # Parse out the disk number from the disk label, e.g. "Hard disk 2"
            disk_num = 0
            match = re.search(".*\s(\d*)", device.deviceInfo.label)
            if match:
                disk_num = int(match.group(1))
        ...
        for device in hardware_devices:
            if device.__class__.__name__ == "VirtualDisk":
                build_disks(device, disks)
            elif device.__class__.__name__ in ['VirtualLsiLogicController',
                                               'VirtualLsiLogicSASController',
                                               'VirtualBusLogicController',
                                               'ParaVirtualSCSIController']:
                # If no disk attached, controller has no device attribute.
                if hasattr(device, 'device'):
                    for key in device.device:
                        disk_scsi_controller_mapping[key] = device
        ...
        for disk in disks:
            if disk['in_vm_folder']:
                vm_disks += 1
                if vm_disks == 1:
                    root_disk['root_device_name'] = self._get_disk_device_name(disk['disk_num'])
                    root_disk['device'] = disk['device']
                    root_disk['is_scsi'] = disk['key'] in disk_scsi_controller_mapping
                    break
