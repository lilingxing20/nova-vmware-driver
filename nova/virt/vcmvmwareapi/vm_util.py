...
## in Mitaka Newton
## Does not define the function. This might be a bug
...
def get_stats_from_cluster(session, cluster=None, resource_pool=None):
    ### Vsettan-add cluster, resource_pool
...
def get_vmdk_volume_disk(hardware_devices, path=None):
 ...
 # Vsettan-only start
def get_res_pool_ref(session, cluster, host=None, res_pool_name=None):
    ## Vsettan-add host, res_pool_name
...
def get_stats_from_host(session, host_mor=None):
    ...
    if summary and summary.quickStats != '':
        ...
        stats["hypervisor_version"] = nova_utils.convert_version_to_int(
