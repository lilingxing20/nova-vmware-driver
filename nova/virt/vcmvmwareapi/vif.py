...
vmwareapi_vif_opts = [
    ...
    cfg.BoolOpt('ignore_vswitch_validation',
               default=False,
               help='If True then do not check if the port group and'
                    ' vlan interface are associated on the same virtual'
                    ' switch'),
    cfg.BoolOpt('ignore_portgroup_create',
               default=False,
               help='If True then do not create the port group when using'
                    ' nova-network'),
    # Vsettan-only end
]
...
def _get_associated_vswitch_for_interface(session, interface, cluster=None,
    ...
    if not network_util.check_if_vlan_interface_exists(session,
                                        interface, cluster, host):  #Vsettan-only
    ...
    vswitch_associated = network_util.get_vswitch_for_vlan_interface(
                                    session, interface, cluster, host)  #Vsettan-only
...
def ensure_vlan_bridge(session, vif, cluster=None,
    ...
    network_ref = network_util.get_network_with_the_name(session, bridge,
                                                         cluster, host)  #Vsettan-only
    if network_ref and network_ref['type'] == 'DistributedVirtualPortgroup':
        return network_ref

    if not network_ref:
        # Vsettan-only start
        ignore_portgroup_create = CONF.vmware.ignore_portgroup_create
        if ignore_portgroup_create:
            LOG.warning(_("Cannot find the network %(net)s and network creation"
                          " is disabled. Please create the network on the"
                          " %(host)s manually."), {'net': bridge, 'host': host})
        ...
        vswitch_associated = _get_associated_vswitch_for_interface(session,
                                 vlan_interface, cluster, host)  #Vsettan-only
        ...
        # Vsettan-only start
        ignore_vswitch_validation = CONF.vmware.ignore_vswitch_validation
        if ignore_vswitch_validation:
            LOG.info(_("Found network '%s', skipping parent virtual switch"
                       " validate."), bridge)
            return network_ref
        # Vsettan-only end
...
def _get_opaque_network(session, cluster, host):  #Vsettan-only
    #Vsettan-only start
    if host is None:
...
def get_neutron_network(session, network_name, cluster, host, vif):  #Vsettan-only
    opaque = None
    if vif['type'] != model.VIF_TYPE_DVS:
        opaque = _get_opaque_network(session, cluster, host)   #Vsettan-only
    if opaque:
        ...
        network_ref = network_util.get_network_with_the_name(
                session, network_name, cluster, host)   #Vsettan-only
    if not network_ref:
        raise exception.NetworkNotFoundForBridge(bridge=bridge)
    return network_ref


def get_network_ref(session, cluster, host, vif, is_neutron):  #Vsettan-only
    if is_neutron:
        ...
        network_ref = ensure_vlan_bridge(session, vif, cluster=cluster,
                                         host=host,   #Vsettan-only
                                         create_vlan=create_vlan)
    return network_ref


def get_vif_dict(session, cluster, host, vif_model, is_neutron, vif): #Vsettan-only
    mac = vif['address']
    name = vif['network']['bridge'] or CONF.vmware.integration_bridge
    ref = get_network_ref(session, cluster, host, vif, is_neutron) #Vsettan-only
...
def get_vif_info(session, cluster, host, is_neutron, vif_model, network_info): #Vsettan-only
    vif_infos = []
    if network_info is None:
        return vif_infos
    for vif in network_info:
        # Vsettan-only start
        if "Discovered Network " in vif['network']['label']:
            vif['network']['bridge'] = vif['network']['label'][len("Discovered Network "):]
        elif vif['type'] == model.VIF_TYPE_OVS or vif['type'] == 'binding_failed':
            vif['network']['bridge'] = vif['network']['label']
        # Vsettan-only stop
        vif_infos.append(get_vif_dict(session, cluster,
                         host,   #Vsettan-only
                         vif_model,
                         is_neutron, vif))
    return vif_infos
